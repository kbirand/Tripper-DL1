...
void loop() {
  ...
  if (now - tTele >= 200) {
    ...
    // appends accDev_mg, the window's peak departure from 1 g; 0x07 appends
    // canOdo_km, the bike's own odometer.
    p.ver = PKT_VER;
  ...
  if (now - tStatus >= 1000) {
    ...
    logLine("[dbg] fix=%d sats=%d view=%d conn=%d R=%+.1f P=%+.1f gz=%+.1f cal=%d%d%d qrej=%lu baro=%.1fm mark=%d btn=%d%d lps=%lu stall=%lu can=%s/%lu %u%% %.1fV ovr=%u/%u tx=%lu/%lu %s txerr=%u rxerr=%u q=%lu boff=%lu",
